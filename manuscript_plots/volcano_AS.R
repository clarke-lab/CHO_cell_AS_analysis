...
load("rMats_output/SE.rData")
SE <- coverage_filtered

load("rMats_output/MXE.rData")
MXE <- coverage_filtered

load("rMats_output/RI.rData")
RI <- coverage_filtered

load("rMats_output/A5SS.rData")
A5SS <- coverage_filtered

load("rMats_output/A3SS.rData")
A3SS <- coverage_filtered
...
AS_event <- data.frame(
  type = c(SE$type, MXE$type, RI$type, A5SS$type, A3SS$type),
  dPSI = as.numeric(c(
    SE$IncLevelDifference,
...
AS_event <- AS_event %>% mutate(pointcolor = case_when(
  dPSI > 0 & threshold == "TRUE" ~ "#F9B288",
  dPSI < 0 & threshold == "TRUE" ~ "#A2D9F9",
  threshold == "FALSE" ~ "gray"
))
AS_event <- AS_event %>% mutate(pointclass = case_when(
  dPSI > 0 & threshold == "TRUE" ~ "Increase at 31C",
  dPSI < 0 & threshold == "TRUE" ~ "Increase at 37C ",
  threshold == "FALSE" ~ "Not Significant"
))
AS_event <- AS_event %>% mutate(pointsize = case_when(
...
ggsave(paste("AS_results/", "AS_volcano_plot.tiff"),
  plot = g,
  height = 3,
  width = 6,
  units = "in",
